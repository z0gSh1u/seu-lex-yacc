...
import { inStr, splitAndKeep, isAlpha, inRange, assert } from '../../utils'
/**
 * 正则表达式类
 */
export class Regex {
  private _raw: string // 最原始的正则表达式（只做了别名展开）
  private _rangeExpanded!: string // 做了range展开后的正则表达式
  ...
  private _addDots() {
    ...
    for (let i = 1; i < this._rangeExpanded.length; i++) {
      // 前中后三个位置的字符
      let curCh = this._rangeExpanded[i],
        prevCh = this._rangeExpanded[i - 1]
